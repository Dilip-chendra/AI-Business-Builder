...
class AIService:
    ...
    async def generate_text(
        ...
        *,
        ...
        # Build candidate list — skip circuit-open providers
        ...
        candidates = [
            ...
            if provider in configured and telemetry.is_provider_available(provider)
        ]

        for provider in preferred_order:
            if provider in configured and provider not in candidates:
        ...
        raise AIProviderError(
            "No AI provider is available. Configure GROQ_API_KEY or HF_API_KEY, "
    ...
    def _provider_order_for_task(task_type: str | None, *, prefer_json: bool) -> list[str]:
        ...
        if task in {"quick_task", "short_classification", "json_formatting"}:
            return ["groq", "ollama", "huggingface"]
        if task in {"ai_studio_app_builder", "code_edit_fast"}:
            return ["groq", "ollama", "huggingface"]
    ...
    async def _groq_call(self, prompt: str, json_mode: bool = True) -> str:
        ...
        max_tokens = 4096 if json_mode else 3072
        body: dict = {
            "model": model,
            "messages": messages,
            "temperature": 0.35,
    ...
    def _parse_json(self, output: str) -> dict[str, Any]:
        ...
        balanced = self._extract_first_json_object(cleaned)
        if balanced:
            cleaned = balanced
        try:
            ...
            if repaired:
                try:
                    ...
                    if isinstance(parsed, dict):
                        logger.warning("Repaired truncated AI JSON response before applying it.")
                        return parsed
            ...
            balanced = self._extract_first_json_object(match.group())
            object_text = balanced or match.group()
            try:
                parsed = json.loads(object_text)
            except json.JSONDecodeError as exc:
                repaired = self._repair_truncated_json(object_text)
                if repaired:
                    try:
                        parsed = json.loads(repaired)
                    except json.JSONDecodeError:
                        ...
                        if isinstance(parsed, dict):
                            logger.warning("Repaired extracted truncated AI JSON response before applying it.")
                            return parsed
                raise AIProviderError(
    ...
    def _extract_first_json_object(text: str) -> str | None:
        """Return the first complete top-level JSON object from mixed model text."""
        start = text.find("{")
        if start < 0:
            return None
        depth = 0
    ...
    def _repair_truncated_json(text: str) -> str | None:
        """Best-effort repair for provider output cut off mid JSON object."""
        if "{" not in text:
            return None
    ...
    def _business_prompt(self, payload: BusinessGenerateRequest) -> str:
        return (
            ...
            "PRODUCT INTELLIGENCE GUIDANCE FROM THE PRODUCTION HARDENING REPORT:\n"
            "- The strongest first market is residential home services: HVAC, plumbing, electrical, and roofing operators under 20 employees.\n"
            "- When user input fits local services, create an execution-focused business around booked calls, quote requests, review generation, fast follow-up, scheduling, and real revenue attribution.\n"
            "- Do not position the idea as a generic AI content tool when an operating-system workflow is possible.\n"
            "- If the user input clearly asks for another niche, follow that niche while still emphasizing real workflows, approval gates, and measurable business outcomes.\n\n"
