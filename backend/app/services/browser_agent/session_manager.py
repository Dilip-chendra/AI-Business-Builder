...
SOCIAL_STEALTH_SKIP_TOKENS = ("linkedin", "instagram", "facebook")
SOCIAL_STEALTH_SKIP_DOMAINS = ("linkedin.com", "instagram.com", "facebook.com")
...
class SessionManager:
    ...
    def __init__(self, *, headless: bool | None = None, session_id: str | None = None) -> None:
        ...
        root = Path(settings.resolved_browser_session_root)
        ...
        lowered_session = self.session_id.lower()
        self._stealth_enabled = not any(token in lowered_session for token in SOCIAL_STEALTH_SKIP_TOKENS)
    ...
    def _wire_page(self, page: Page) -> None:
        ...
        def _on_page_error(exc: Exception) -> None:
            message = str(exc)
            if (
                "sj_evt is not defined" in message
                or "Cannot redefine property: offsetHeight" in message
                or "Cannot assign to read only property 'createElement'" in message
            ):
                return
    ...
    async def _apply_stealth(self, page: Page) -> None:
        ...
        if not self._stealth_enabled or any(domain in page_url for domain in SOCIAL_STEALTH_SKIP_DOMAINS):
            setattr(page, "_autonomous_browser_stealth_applied", True)
            return
        try:
            await self._stealth.apply_stealth_async(page)
        except Exception:
            logger.debug("Stealth patch skipped for page=%s session=%s", page.url, self.session_id, exc_info=True)
        setattr(page, "_autonomous_browser_stealth_applied", True)
