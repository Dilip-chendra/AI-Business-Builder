...
class BrowserAgent:
    ...
    async def _iter_events_inline(self, goal: str) -> AsyncGenerator[dict[str, Any], None]:
        ...
        task_plan = self.planner.describe_task(goal)
        ...
        try:
            ...
            while True:
                if step_number > hard_budget:
                    ...
                    if self._has_synthesis_ready_evidence(progress):
                        summary = await self._synthesize_final(goal=goal, progress=progress)
                        result.finish(status="done", result=summary)
                        browser_run_manager.update_snapshot(result.run_id, result_preview=summary)
                        final_state = await self._collect_state()
                    ...
                    summary = await self._synthesize_final(goal=goal, progress=progress)
                    result.finish(status="done" if self._has_synthesis_ready_evidence(progress) else "needs_more_steps", result=summary)
                    browser_run_manager.update_snapshot(result.run_id, result_preview=summary)
                    final_state = await self._collect_state()
                    yield {
                        "type": "result",
                ...
                if duplicate_reason:
                    ...
                    if second_duplicate_reason:
                        current_url = str(state["url"] or "")
                        if (
                            current_url
                            and not self.planner._is_search_page(current_url)
                            and not self.memory.has_extracted_url(current_url)
                        ):
                            action = {
                                "action": "extract",
                                "instruction": self.planner._build_extract_instruction(goal_profile, goal),
                                "reason": "Loop prevention detected repeated navigation, so extract the current source now instead of searching again.",
                            }
                        elif self.memory.structured_evidence(limit=1):
                            ...
                            break
                        elif not self._should_finalize(progress):
                            action = {
                                "action": "search",
                ...
                current_url = str(state["url"] or "")
                if (
                    action_name in {"scroll", "search", "switch_tab", "open_tab"}
                    and current_url
                    and not self.planner._is_search_page(current_url)
                    and not self.memory.has_extracted_url(current_url)
                ):
                    action = {
                        "action": "extract",
                        "instruction": self.planner._build_extract_instruction(goal_profile, goal),
                        "reason": "The current source has not been extracted yet, so capture evidence before navigating, scrolling, or searching again.",
                    }
                ...
                current_progress = self.memory.research_progress(goal=goal, target_sources=target_sources)
                current_phase = self._determine_phase(next_state["url"], current_progress, bool(self.memory.evidence), False)
                current_evidence = self.memory.structured_evidence(limit=5)
            ...
            yield {
                ...
                "sources": result.sources,
                "phase": "completed" if result.status in {"done", "completed"} else run_state.last_phase,
                "progress": self.memory.research_progress(goal=goal, target_sources=target_sources),
    ...
    @staticmethod
    def _has_synthesis_ready_evidence(progress: dict[str, Any]) -> bool:
        useful_sources = int(progress.get("useful_sources", 0))
        extracted_sources = int(progress.get("extracted_sources", 0))
        evidence_density = float(progress.get("evidence_density", 0.0))
        structured_density = float(progress.get("structured_density", 0.0))
        avg_quality = float(progress.get("avg_quality", 0.0))
        return (
            useful_sources >= 2
            or extracted_sources >= 3
            or (useful_sources >= 1 and (evidence_density >= 1.0 or structured_density >= 1.0) and avg_quality >= 0.2)
        )
