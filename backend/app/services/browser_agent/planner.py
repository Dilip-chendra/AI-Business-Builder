...
class BrowserPlanner:
    ...
    def describe_task(self, goal: str) -> dict[str, Any]:
        goal_profile = self._goal_profile(goal)
        profile_labels = {
            "keyword_research": "SEO keyword research",
            "competitor_research": "Competitor analysis",
            "pricing_research": "Pricing analysis",
            "publishing": "Publishing workflow",
            "general_research": "Market research",
        }
        objective_map = {
            "keyword_research": [
                "Find distinct, relevant sources about the target niche.",
                "Extract keyword clusters, intent themes, SEO opportunities, and content angles.",
                "Compare patterns across sources and produce recommended actions.",
            ],
            "competitor_research": [
                "Identify credible competitor or comparison sources.",
                "Extract positioning, feature, audience, pricing, and differentiation signals.",
                "Synthesize competitive gaps and recommended moves.",
            ],
            "pricing_research": [
                "Collect multiple pricing and packaging references.",
                "Extract prices, tiers, billing periods, free trials, and differentiators.",
                "Summarize pricing patterns and recommend offer structure.",
            ],
            "publishing": [
                "Open the target platform safely.",
                "Prepare the draft and wait for final user confirmation.",
                "Record the publishing outcome.",
            ],
            "general_research": [
                "Search for credible, diverse sources.",
                "Extract useful facts, trends, entities, and evidence.",
                "Synthesize findings into a business-ready report.",
            ],
        }
        completion_map = {
            "publishing": [
                "Draft is prepared.",
                "Final confirmation is requested before irreversible publish actions.",
                "Outcome is reported and persisted.",
            ],
            "default": [
                "Enough unique sources have been extracted.",
                "Evidence has acceptable relevance and quality.",
                "A structured final report is generated and persisted.",
            ],
        }
        return {
            "profile": goal_profile,
            "label": profile_labels.get(goal_profile, "Research task"),
            "strategy": self._focused_search_query(goal, goal_profile),
            "objectives": objective_map.get(goal_profile, objective_map["general_research"]),
            "completion_criteria": completion_map["publishing" if goal_profile == "publishing" else "default"],
        }
    ...
    def deterministic_research_action(
        ...
        *,
        ...
        current_key = BrowserMemory.normalize_url(current_url).lower()
        ...
        if current_url and not self._is_search_page(current_url):
            if current_key not in extracted_urls and current_key not in rejected_urls:
                if self._page_has_useful_content(goal_profile, title, headings, text_excerpt):
                    return {
                        ...
                        "reason": "The current source page is relevant and has not been extracted yet, so capture structured evidence before switching tabs.",
                    }
                if self._looks_like_extractable_source(dom_snapshot):
                    return {
                        "action": "extract",
                        "instruction": self._build_extract_instruction(goal_profile, goal),
                        "reason": "This is an unextracted source page, so capture its visible article/content before deciding whether more browsing is needed.",
                    }
        ...
        if source_tab is not None:
            ...
            if current_url and not self._is_search_page(current_url):
                action["reject_current_source"] = True
                action["rejected_url"] = current_url
                action["reason"] = "The current source does not look useful enough to extract, so reject it and inspect another open source tab."
            return action
            ...
            if useful_sources < target_sources:
                ...
                if search_tab is not None:
                    return {
                        ...
                        "reject_current_source": current_key not in extracted_urls,
                        "rejected_url": current_url if current_key not in extracted_urls else "",
    ...
    def _fallback_summary(
        *,
        ...
        for item in source_records[:6]:
            ...
            for keyword in data.get("keyword_candidates", [])[:8]:
                cleaned_keyword = str(keyword).strip()
                if cleaned_keyword and cleaned_keyword not in keyword_signals:
                    keyword_signals.append(cleaned_keyword[:140])
            for entity in data.get("entities", [])[:6]:
                cleaned_entity = str(entity).strip()
                if cleaned_entity and cleaned_entity not in entity_signals:
                    entity_signals.append(cleaned_entity[:120])
            for feature in data.get("features", [])[:5]:
                cleaned_feature = str(feature).strip()
                if cleaned_feature and cleaned_feature not in feature_signals:
                    feature_signals.append(cleaned_feature[:220])
            for plan in data.get("plan_clues", [])[:4]:
                cleaned_plan = str(plan).strip()
                if cleaned_plan and cleaned_plan not in plan_signals:
                    plan_signals.append(cleaned_plan[:220])
            for link in data.get("important_links", [])[:4]:
                text = str((link or {}).get("text") or "").strip()
                href = str((link or {}).get("href") or "").strip()
                if text and href:
                    line = f"{text}: {href}"
                    if line not in link_signals:
                        link_signals.append(line[:220])
            if len(key_findings) >= 8:
                break
        ...
        if feature_signals:
            extracted_lines.append(f"- Product or market feature signals: {'; '.join(feature_signals[:4])}")
        if link_signals:
            extracted_lines.append(f"- Useful follow-up links: {'; '.join(link_signals[:4])}")
        ...
        return "\n".join(
            [
                ...
                *source_lines,
                "",
                "## Competitor Insights",
                *( [f"- Entities and competitor-like names observed: {', '.join(entity_signals[:10])}"] if entity_signals else ["- No clear competitor or brand entities were extracted from the captured sources."] ),
                *( [f"- Repeated feature and positioning signals: {'; '.join(feature_signals[:5])}"] if feature_signals else ["- Feature and positioning extraction was limited; continue research for stronger competitor-specific evidence."] ),
                "",
                "## SEO and Marketing Insights",
                *( [f"- Keyword and topic candidates: {', '.join(keyword_signals[:14])}"] if keyword_signals else ["- No explicit keyword candidates were extracted from the captured sources."] ),
                *[line for line in extracted_lines if "Pricing signals" not in line],
                "",
                "## Pricing Findings",
                *( [f"- Pricing signals captured: {', '.join(price_points[:8])}"] if price_points else ["- No explicit pricing signals were extracted from the captured sources."] ),
                *( [f"- Plan and package clues: {'; '.join(plan_signals[:6])}"] if plan_signals else [] ),
                "",
                "## Recommended Actions",
                *recommendations,
                "",
                "## Research Confidence Score",
                f"- Confidence: {round(float(progress.get('progress_score', 0.0)) * 100)}%",
                f"- Source coverage: {progress.get('unique_sources', 0)} of {progress.get('target_sources', 0)} target sources.",
                f"- Coverage met: {'yes' if progress.get('coverage_met') else 'no'}",
            ]
        )
    ...
    def _page_has_useful_content(
        ...
        if len(text_excerpt) > 320 and any(token in haystack for token in ("2025", "2026", "best", "top", "guide", "ideas", "products", "tools", "niche", "market", "trend", "pricing", "keywords")):
            return True
        if goal_profile == "keyword_research":
    ...
    @staticmethod
    def _looks_like_extractable_source(dom_snapshot: dict[str, Any]) -> bool:
        url = str(dom_snapshot.get("url") or "").lower()
        title = str(dom_snapshot.get("title") or "").lower()
        if any(token in url for token in ("duckduckgo.com", "bing.com", "google.", "accounts.", "login", "signin")):
            return False
        blocker_blob = " ".join(str(item).lower() for item in dom_snapshot.get("blockers", []))
        if blocker_blob:
            return False
        text_excerpt = str(dom_snapshot.get("visible_text") or dom_snapshot.get("text_excerpt") or "")
        headings = dom_snapshot.get("headings") or []
        if len(text_excerpt) >= 120:
            return True
        if len(headings) >= 2 and not any(token in title for token in ("just a moment", "security check", "captcha")):
            return True
        return False
