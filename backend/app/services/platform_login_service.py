...
class PlatformLoginService:
    ...
    async def ensure_session(self, *, platform: str, credentials: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if platform in {"gmail", "google"}:
            return await self._ensure_gmail_session(credentials)
    ...
    async def _ensure_gmail_session(self, credentials: dict[str, Any]) -> dict[str, Any]:
        page = await self.session_manager.get_page()
        login_identifier = self._best_login_identifier(credentials)
        password = credentials.get("password")
        if not login_identifier or not password:
            return {"status": "failed", "message": "Saved Gmail credentials are incomplete."}

        try:
            await self._safe_goto(page, "https://mail.google.com/mail/u/0/#inbox")
            await self._pause(1.4)
            if await self._gmail_logged_in():
                await self.session_manager.persist_storage_state()
                return {"status": "connected", "message": "Gmail session is already active."}

            await self._safe_goto(page, "https://accounts.google.com/signin/v2/identifier?service=mail")
            email_locator = await self._find_first_locator(
                page,
                ["input[type='email']", "#identifierId", "input[name='identifier']"],
                timeout_ms=20000,
            )
            await email_locator.fill(str(login_identifier))
            await self._click_google_next(page)
            await self._pause(1.4)

            if await page.locator("input[type='password'], input[name='Passwd']").count():
                password_locator = await self._find_first_locator(
                    page,
            ...
            logger.warning("Timed out during Gmail login bootstrap for session %s", self.session_manager.session_id)
            return {"status": "failed", "message": "Gmail login timed out before the session could be restored."}
        except Exception:
            logger.exception("Gmail login bootstrap failed for session %s", self.session_manager.session_id)
            return {"status": "failed", "message": "Gmail could not complete the automated login flow."}

        if await self._wait_for_authenticated_state(self._gmail_logged_in, timeout_seconds=24):
            await self.session_manager.persist_storage_state()
            return {"status": "connected", "message": "Gmail session restored successfully."}
        if await page.locator("input[type='tel'], input[autocomplete='one-time-code'], input[name='totpPin']").count():
            return {"status": "failed", "message": "Gmail is asking for a verification code before the session can continue."}
        return {"status": "failed", "message": "Gmail needs additional verification before the browser draft workflow can continue."}
    ...
    async def _gmail_logged_in(self) -> bool:
        page = await self.session_manager.get_page()
        if "mail.google.com" in page.url and await page.locator("div[role='button']:has-text('Compose'), a[title*='Inbox'], div[gh='cm']").count():
            return True
        return False

    async def _click_google_next(self, page: Page) -> None:
        locator = await self._find_first_locator(
            page,
            ["button:has-text('Next')", "#identifierNext button", "#passwordNext button", "div[role='button']:has-text('Next')"],
            timeout_ms=12000,
        )
