...
def _project_matches_business(project: Project | None, business: Business) -> bool:
    if not project:
        return False
    business_token = "".join(ch for ch in (business.name or "").lower() if ch.isalnum())
    project_token = "".join(ch for ch in (project.name or "").lower() if ch.isalnum())
    return bool(business_token and business_token in project_token)
...
class ContextService:
    ...
    async def _hydrate_legacy_businesses(self, user: User, workspace_id: str) -> list[Business]:
        ...
        for business in businesses:
            ...
            existing_project = await self.db.get(Project, business.project_id) if business.project_id else None
            needs_project = (
                business.workspace_id
                and str(business.workspace_id) == workspace_id
                and (
                    business.project_id is None
                    or existing_project is None
                    or not _project_matches_business(existing_project, business)
                )
            )
