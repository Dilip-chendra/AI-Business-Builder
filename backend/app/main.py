...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        ...
        "configured" if search_configured else "not_configured",
        "enabled" if settings.enable_browser_agent else "disabled",
    )
    if health.any_available:
        active = [provider for provider in ("groq", "huggingface", "ollama") if getattr(health, provider)]
        logger.info("AI providers available: %s", ", ".join(active))
    else:
        logger.warning(
            "No AI providers configured. Set GROQ_API_KEY or HF_API_KEY in .env, "
