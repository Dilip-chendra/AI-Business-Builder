...
export function EmailPreview({ content }: { content: Record<string, any> }) {
  ...
  const text = cleanDisplayText(content.plain_text || content.plain_text_body || content.body || "");
  const subject = cleanDisplayText(content.subject || "");
  const preview = cleanDisplayText(content.preview_text || "");
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {posts.slice(0, 3).map((post: any, i: number) => (
          <div key={i} style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px", borderLeft: "3px solid #7c3aed" }}>
            <p style={{ fontSize: 13, color: "#0f172a", lineHeight: 1.6, margin: "0 0 6px" }}>{cleanDisplayText(post.text)}</p>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {c.campaign_name && (
        <div style={{ background: "#fef3c7", borderRadius: 10, padding: "10px 12px" }}>
          ...
          <p style={{ fontSize: 14, fontWeight: 700, color: "#92400e", margin: 0 }}>{cleanDisplayText(c.campaign_name)}</p>
        </div>
      )}
      {c.ad_creatives?.slice(0, 2).map((ad: any, i: number) => (
        <div key={i} style={{ background: "#f8fafc", borderRadius: 10, padding: "10px 12px" }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", margin: "0 0 4px" }}>{cleanDisplayText(ad.headline)}</p>
          <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 4px" }}>{cleanDisplayText(ad.description)}</p>
          {ad.cta && <span style={{ fontSize: 11, fontWeight: 700, color: "#d97706", background: "#fef3c7", padding: "2px 8px", borderRadius: 99 }}>{cleanDisplayText(ad.cta)}</span>}
        </div>
      ))}
