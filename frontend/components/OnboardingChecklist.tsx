...
export function OnboardingChecklist() {
  ...
  const syncedSteps = status?.steps.map((step) =>
    step.id === "create_first_business" && businesses.length > 0
      ? { ...step, completed: true }
      : step
  ) || [];
  const syncedCompleted = syncedSteps.filter((step) => step.completed).length;
  const hasOnlyLegacyFirstBusinessGap =
    businesses.length > 0 &&
    status &&
    status.completed_count < status.total_count &&
    status.steps.some((step) => step.id === "create_first_business" && !step.completed);

  if (dismissed || !status || status.onboarding_complete || hasOnlyLegacyFirstBusinessGap) return null;

  const progress = Math.round((syncedCompleted / status.total_count) * 100);
    ...
    <div style={{
      ...
      <div style={{ padding: "14px 16px", background: "linear-gradient(135deg,#0f172a,#1e1b4b)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", margin: 0 }}>{syncedCompleted}/{status.total_count} steps complete</p>
        </div>
