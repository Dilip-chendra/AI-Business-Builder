...
const SCHEMES: Record<string,{primary:string;light:string;glow:string;dark:string;hero:string;ink:string}> = {
  indigo:  {primary:"#6366f1",light:"#ede9fe",glow:"rgba(99,102,241,0.35)",dark:"#4f46e5",hero:"linear-gradient(135deg,#0f172a 0%,#1e1b4b 58%,#111827 100%)",ink:"#fff"},
  emerald: {primary:"#10b981",light:"#d1fae5",glow:"rgba(16,185,129,0.35)",dark:"#059669",hero:"linear-gradient(135deg,#052e2b 0%,#065f46 58%,#0f172a 100%)",ink:"#ecfdf5"},
  rose:    {primary:"#f43f5e",light:"#ffe4e6",glow:"rgba(244,63,94,0.35)",dark:"#e11d48",hero:"linear-gradient(135deg,#3f0a1f 0%,#9f1239 58%,#111827 100%)",ink:"#fff1f2"},
  amber:   {primary:"#f59e0b",light:"#fef3c7",glow:"rgba(245,158,11,0.35)",dark:"#b45309",hero:"linear-gradient(135deg,#1c1204 0%,#92400e 56%,#f59e0b 100%)",ink:"#fff7ed"},
  black_gold: {primary:"#f5c451",light:"#fef3c7",glow:"rgba(245,196,81,0.38)",dark:"#8a5a0a",hero:"linear-gradient(135deg,#020617 0%,#111827 52%,#8a5a0a 100%)",ink:"#fff8df"},
  sky:     {primary:"#0ea5e9",light:"#e0f2fe",glow:"rgba(14,165,233,0.35)",dark:"#0284c7",hero:"linear-gradient(135deg,#082f49 0%,#0369a1 58%,#0f172a 100%)",ink:"#f0f9ff"},
  violet:  {primary:"#8b5cf6",light:"#ede9fe",glow:"rgba(139,92,246,0.35)",dark:"#7c3aed",hero:"linear-gradient(135deg,#1e1b4b 0%,#5b21b6 58%,#111827 100%)",ink:"#f5f3ff"},
};
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/businesses/${id}/${previewMode ? "landing-page-preview" : "landing-page"}?fresh=${encodeURIComponent(freshKey)}`, { cache: "no-store" }).then(r => r.ok ? r.json() : Promise.reject()),
      fetch(`${API_URL}/businesses/${id}/products-public?fresh=${encodeURIComponent(freshKey)}`, { cache: "no-store" }).then(r => r.ok ? r.json() : []).catch(() => []),
    ])
      .then(([b, p]) => {
        setBiz(b);
  ...
  const leadCapture: any = pc.lead_capture || null;
  const quoteRequest: any = pc.quote_request || null;
  return (
    <div style={{minHeight:"100vh",background:"#f8fafc",fontFamily:"-apple-system,BlinkMacSystemFont,Inter,sans-serif"}}>
      ...
      {/* HERO */}
      <section style={{background:scheme.hero,padding:"90px 24px 110px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-80,right:-80,width:400,height:400,borderRadius:"50%",background:`${scheme.primary}38`,filter:"blur(80px)"}}/>
        <div style={{position:"absolute",bottom:-60,left:60,width:300,height:300,borderRadius:"50%",background:`${scheme.primary}24`,filter:"blur(60px)"}}/>
        <div style={{maxWidth:780,margin:"0 auto",textAlign:"center",position:"relative"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:`${scheme.primary}25`,color:scheme.ink,borderRadius:99,padding:"6px 16px",fontSize:12,fontWeight:700,marginBottom:20,border:`1px solid ${scheme.primary}70`}}>
            <Sparkles size={12}/> {biz.niche}
          </div>
          <h1 style={{fontSize:"clamp(34px,5.5vw,62px)",fontWeight:900,color:scheme.ink,margin:"0 0 20px",lineHeight:1.1,letterSpacing:"-0.02em"}}>{biz.headline}</h1>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <a id="cta" href={pricingTiers.length>0?"#pricing":"#about"} style={{display:"inline-flex",alignItems:"center",gap:8,background:`linear-gradient(135deg,${scheme.primary},${scheme.dark})`,color:"#fff",fontWeight:800,fontSize:16,padding:"15px 32px",borderRadius:14,textDecoration:"none",boxShadow:`0 8px 30px ${scheme.glow}`,border:"1px solid rgba(255,255,255,0.22)"}}>
      ...
      {(leadCapture || quoteRequest)&&(
        <section style={{background:"#fff",padding:"76px 24px"}}>
          <div style={{maxWidth:1100,margin:"0 auto"}}>
            <p style={{textAlign:"center",fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:scheme.primary,marginBottom:12}}>Start the conversation</p>
            <h2 style={{textAlign:"center",fontSize:"clamp(24px,3vw,36px)",fontWeight:800,color:"#0f172a",margin:"0 0 42px"}}>Turn interest into a booked next step</h2>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20}}>
              {leadCapture&&(
                <div style={{background:"#f8fafc",borderRadius:20,border:`1px solid ${scheme.primary}35`,padding:"28px 26px",boxShadow:"0 18px 44px rgba(15,23,42,0.07)"}}>
                  <p style={{fontSize:11,fontWeight:800,letterSpacing:"0.1em",textTransform:"uppercase",color:scheme.primary,margin:"0 0 10px"}}>Lead capture</p>
                  <h3 style={{fontSize:24,fontWeight:900,color:"#0f172a",margin:"0 0 10px"}}>{leadCapture.headline || "Get a Fast Response"}</h3>
                  <p style={{fontSize:14,color:"#64748b",lineHeight:1.7,margin:"0 0 18px"}}>{leadCapture.description}</p>
                  <div style={{display:"grid",gap:10,marginBottom:18}}>
                    {(leadCapture.fields||["Name","Email","Phone"]).map((field:string)=>(
                      <div key={field} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"12px 14px",fontSize:13,color:"#94a3b8"}}>{field}</div>
                    ))}
                  </div>
                  <button style={{width:"100%",padding:"13px 16px",borderRadius:12,border:"none",background:`linear-gradient(135deg,${scheme.primary},${scheme.dark})`,color:"#fff",fontWeight:800,fontFamily:"inherit"}}>{leadCapture.cta || biz.cta_text}</button>
                </div>
              )}
              {quoteRequest&&(
                <div style={{background:"#0f172a",borderRadius:20,border:`1px solid ${scheme.primary}70`,padding:"28px 26px",boxShadow:`0 18px 50px ${scheme.glow}`}}>
                  <p style={{fontSize:11,fontWeight:800,letterSpacing:"0.1em",textTransform:"uppercase",color:scheme.primary,margin:"0 0 10px"}}>Quote request</p>
                  <h3 style={{fontSize:24,fontWeight:900,color:"#fff",margin:"0 0 10px"}}>{quoteRequest.headline || "Request a Clear Quote"}</h3>
                  <p style={{fontSize:14,color:"rgba(255,255,255,0.68)",lineHeight:1.7,margin:"0 0 18px"}}>{quoteRequest.description}</p>
                  <div style={{display:"grid",gap:10,marginBottom:18}}>
                    {(quoteRequest.fields||["Service needed","Timeline","Phone"]).map((field:string)=>(
                      <div key={field} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"12px 14px",fontSize:13,color:"rgba(255,255,255,0.55)"}}>{field}</div>
                    ))}
                  </div>
                  <button style={{width:"100%",padding:"13px 16px",borderRadius:12,border:"none",background:`linear-gradient(135deg,${scheme.primary},${scheme.dark})`,color:"#fff",fontWeight:800,fontFamily:"inherit"}}>{quoteRequest.cta || biz.cta_text}</button>
                </div>
              )}
            </div>
          </div>
