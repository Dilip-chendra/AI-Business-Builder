...
async function readApiError(response: Response, fallback: string): Promise<Error> {
  try {
    ...
    return new Error(actionErrorMessage(payload, fallback));
  } catch {
...
export default function CodeEditorPage() {
  ...
  const [aiEditSaved, setAiEditSaved] = useState(false);
  ...
  useEffect(() => {
    if (active.business_id) {
      setSelectedBusinessId(active.business_id);
      return;
    }
    if (businesses[0]) {
  ...
  async function loadFiles(businessId?: string) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        throw await readApiError(r, "Could not load real project files for this business.");
      }
    } catch (e: any) { setError(actionErrorMessage(e, "Could not load real project files for this business.")); }
  }
  ...
  async function openFile(path: string, businessId?: string) {
    ...
    try {
      ...
      else { throw await readApiError(r, `Could not open ${path}.`); }
      await loadVersions(path, bid);
    } catch (e: any) { setError(actionErrorMessage(e, `Could not open ${path}.`)); }
  }
  ...
  async function saveFile() {
    ...
    try {
      ...
      if (!r.ok) throw await readApiError(r, "Could not save the selected file.");
      setSaved(true);
      setOriginalCode(code);
      setPreviewVersion(Date.now());
      setTimeout(() => setSaved(false), 2000);
      await loadVersions(selectedFile);
    } catch (e: any) { setError(actionErrorMessage(e, "Could not save the selected file.")); } finally { setSaving(false); }
  }
  ...
  function buildAgentPlan(instr: string): string[] {
    const relatedFiles = files
      .filter((file) => file.path !== selectedFile)
      .slice(0, 4)
      .map((file) => file.path);
    return [
      `Inspect ${selectedFile || "the selected file"} and preserve the current project structure.`,
      relatedFiles.length > 0
        ? `Use workspace context from ${relatedFiles.join(", ")} before editing.`
        : "Use the active workspace context before editing.",
      `Generate a real code diff for: ${instr}`,
      `Apply the accepted diff to ${selectedFile || "the real workspace file"} and save a version snapshot.`,
      "Reload file state and preview data from the backend after save.",
    ];
  }
  ...
  async function applyAIEdit() {
    ...
    if (isMultiStep(instruction) && !agentPlanApproved) {
      setAgentPlanSteps(buildAgentPlan(instruction));
    ...
    try {
      ...
      if (!r.ok) throw await readApiError(r, "AI edit failed.");
      const d = await r.json();
      setUpdatedCode(d.updated_code);
      setAiEditSaved(Boolean(d.saved));
      ...
      setChatHistory(prev => [...prev, { role: "ai", text: d.saved ? `${d.explanation} Saved to ${d.path} as version ${d.version_number || "latest"}.` : d.explanation }]);
    } catch (e: any) {
      const message = actionErrorMessage(e, "AI edit failed.");
      setError(message);
      setChatHistory(prev => [...prev, { role: "ai", text: `Error: ${message}` }]);
    } finally { setLoading(false); setInstruction(""); }
  }

  function streamAIEdit() {
    if (!instruction.trim() || !code || streaming) return;
    setStreaming(true); setStreamStatus("Connecting to workspace..."); setError(null); setStreamedCode(""); setUpdatedCode(null);
    ...
    es.onmessage = (e) => {
      ...
      if (evt.type === "phase") {
        setStreamStatus(evt.message || evt.phase || "Working...");
      }
      ...
      if (evt.type === "complete") {
        ...
        setStreamStatus("");
        setView("diff");
        setChatHistory(prev => [...prev, { role: "ai", text: `Streaming edit complete for ${evt.path || selectedFile}. Review the diff below.` }]);
    ...
    es.onerror = () => { setStreamStatus(""); setStreaming(false); es.close(); esRef.current = null; };
    setInstruction("");
  }
  ...
  async function applyChanges() {
    ...
    if (aiEditSaved) {
      ...
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
      setChatHistory(prev => [...prev, { role: "ai", text: "Saved AI edit accepted in the editor. The real project file was already updated by the backend." }]);
      return;
    }
    setSaving(true);
    try {
      ...
      if (!r.ok) {
        throw await readApiError(r, "Could not save the applied changes.");
      }
      setOriginalCode(nextCode);
      setSaved(true);
      setPreviewVersion(Date.now());
      ...
      const message = actionErrorMessage(e, "Could not save the applied changes.");
      setError(message);
      setChatHistory(prev => [...prev, { role: "ai", text: `Apply succeeded in the editor, but save failed: ${message}` }]);
    } finally {
      setSaving(false);
    }
  }
  ...
  function rejectChanges() {
    if (aiEditSaved) {
      const savedCode = updatedCode || code;
      setCode(savedCode);
      setOriginalCode(savedCode);
      setChatHistory(prev => [...prev, { role: "ai", text: "Closed the diff. The AI edit was already saved to the real project file." }]);
    } else {
  ...
  async function revertVersion(versionId: string) {
    ...
    try {
      ...
      if (!r.ok) throw await readApiError(r, "Failed to revert.");
      await openFile(selectedFile);
      setPreviewVersion(Date.now());
      setChatHistory(prev => [...prev, { role: "ai", text: "Version restored successfully." }]);
    } catch (e: any) {
      setError(actionErrorMessage(e, "Failed to revert."));
  ...
  async function handleGenerateTests() {
    if (!code || !selectedFile || !selectedBusinessId) return;
    const instr = `Generate and save a real test file for ${selectedFile}`;
    setLoading(true); setError(null); setUpdatedCode(null); setAiEditSaved(false); setStreamedCode("");
    setChatHistory(prev => [...prev, { role: "user", text: instr }]);
    try {
      const tok = getToken();
      const r = await fetch(`${API_URL}/code-editor/generate-tests`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${tok}` },
        body: JSON.stringify({
          path: selectedFile,
      ...
      if (!r.ok) throw await readApiError(r, "Test generation failed.");
      const d = await r.json();
      await loadFiles(selectedBusinessId);
      await openFile(d.path, selectedBusinessId);
      setChatHistory(prev => [...prev, { role: "ai", text: `Created ${d.path} and saved it to the real workspace.` }]);
    } catch (e: any) {
      const message = actionErrorMessage(e, "Test generation failed.");
      setError(message);
      setChatHistory(prev => [...prev, { role: "ai", text: `Error: ${message}` }]);
    } finally { setLoading(false); setInstruction(""); }
  }
  ...
  const livePreviewUrl = selectedBusinessId ? `/landing/${selectedBusinessId}?preview=1&fresh=code-editor-${previewVersion}` : "";
            ...
            <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
              ...
              {(streamedCode || streamStatus) && (
                <div style={{ position: "absolute", bottom: 12, right: 12, background: "rgba(99,102,241,0.9)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 99, display: "flex", alignItems: "center", gap: 5 }}>
                  <Loader2 size={10} /> {streamStatus || "Streaming updated file..."}
                </div>
              )}
            </div>
          )}
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 16, padding: "12px", background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)" }}>
                    <button onClick={applyChanges} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: "#10b981", color: "#fff", border: "none", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                      <CheckCircle size={14} /> {aiEditSaved ? "Accept Saved Edit" : "Apply Changes"}
                    </button>
                    <button onClick={rejectChanges} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: "rgba(239,68,68,0.2)", color: "#f87171", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                      <XCircle size={14} /> {aiEditSaved ? "Close Diff" : "Reject"}
                    </button>
                  </div>
                </>
              )}
            ...
            <div style={{ flex: 1, background: "#1e293b", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              {livePreviewUrl ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.08)", background: "rgba(15,23,42,0.92)" }}>
                    <Eye size={14} color="#93c5fd" />
                    <span style={{ color: "#cbd5e1", fontSize: 12, fontWeight: 800 }}>Live business preview</span>
                    <a href={livePreviewUrl} target="_blank" rel="noreferrer" style={{ marginLeft: "auto", color: "#93c5fd", fontSize: 12, fontWeight: 800, textDecoration: "none" }}>Open</a>
                  </div>
                  <iframe
                    key={livePreviewUrl}
                    src={livePreviewUrl}
                    style={{ width: "100%", flex: 1, border: "none", background: "#fff" }}
                    title="Live business preview"
                  />
                </div>
              ) : currentLang === "html" ? (
                <iframe
