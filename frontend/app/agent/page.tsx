  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {lines.map((line, i) => {
        ...
        if (trimLine.startsWith("- ") || trimLine.startsWith("• ") || trimLine.startsWith("* ")) {
          ...
          return (
            <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              <span style={{ color, fontWeight: 800, fontSize: 16, lineHeight: 1.4, flexShrink: 0 }}>•</span>
        ...
        const boldInline = trimLine.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
        if (boldInline !== trimLine) {
          return <p key={i} style={{ fontSize: 14, color: "#374151", lineHeight: 1.7, margin: 0 }} dangerouslySetInnerHTML={{ __html: boldInline }} />;
        }
        // Plain paragraph
        return <p key={i} style={{ fontSize: 14, color: "#374151", lineHeight: 1.7, margin: 0 }}>{trimLine}</p>;
      })}
    </div>
  );
}
        ...
        {Object.entries(data).map(([k, v]) => (
          <div key={k} style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            <span style={{ fontSize: 13, color: "#0f172a" }}>
              {typeof v === "object" ? <JsonToCards data={v} /> : String(v)}
            </span>
          </div>
        ))}
      </div>
