...
import {
  ...
  Users, TrendingUp, DollarSign, Loader2, Zap, RefreshCw,
...
export default function AnalyticsPage() {
  ...
  async function seedTraffic() {
    ...
    try {
      const events = [
        ...Array(12).fill({ event_type: "visit", source: "organic" }),
        ...Array(7).fill({ event_type: "click", source: "landing" }),
        ...Array(3).fill({ event_type: "conversion", source: "pricing" }),
      ];
      for (const ev of events) {
        await fetch(`${API_URL}/analytics/track`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ business_id: businessId, ...ev }),
        });
        await new Promise(r => setTimeout(r, 50));
      }
      setSeedMsg(`Simulated ${events.length} events: 12 visits, 7 clicks, 3 conversions`);
  ...
  return (
    <ErrorBoundary>
      <div className="anim-fade-in" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ background: "linear-gradient(135deg,#0f172a,#1e1b4b)", borderRadius: 20, padding: "32px 36px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", margin: "0 0 6px", lineHeight: 1.6 }}>
                Analytics populate when visitors land on your page. Simulate test events to see how the dashboard looks.
              </p>
              {seedMsg && <p style={{ fontSize: 12, color: "#34d399", margin: "8px 0 0", fontWeight: 600 }}>&#10003; {seedMsg}</p>}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, flexShrink: 0 }}>
              <button
                onClick={seedTraffic}
                disabled={seeding || !businessId}
                style={{ display: "flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", color: "#fff", border: "none", borderRadius: 12, padding: "12px 20px", fontSize: 14, fontWeight: 700, cursor: seeding ? "not-allowed" : "pointer", fontFamily: "inherit", boxShadow: "0 4px 20px rgba(99,102,241,0.4)", opacity: seeding ? 0.7 : 1 }}
              >
                {seeding ? <Loader2 size={16} className="animate-spin" /> : <Zap size={16} />}
                {seeding ? "Simulating traffic..." : "Simulate Test Traffic"}
              </button>
              {businessId && (
                <a
        ...
        {seedMsg && hasData && (
          <div style={{ background: "#f0fdf4", borderRadius: 12, border: "1px solid #bbf7d0", padding: "12px 16px", fontSize: 13, color: "#16a34a", fontWeight: 600 }}>
            &#10003; {seedMsg} — Analytics updated above
          </div>
        )}
