...
const PROVIDERS = [
  {
    key: "featherless" as const,
    label: "Featherless",
    desc: "Primary reasoning and coding provider. Configure FEATHERLESS_API_KEY in backend/.env.",
    icon: BrainCircuit,
    priority: 1,
    setup: "Add FEATHERLESS_API_KEY plus your preferred model IDs in backend/.env.",
    url: "https://featherless.ai/docs/application-guides",
  ...
  },
  {
    key: "groq" as const,
    label: "Groq",
    desc: "Fast path for tiny prompts and quick JSON tasks.",
    icon: Zap,
    priority: 2,
...
export default function AIStatusPage() {
  ...
  const available = health
    ? [health.featherless, health.groq, health.huggingface, health.ollama].filter(Boolean).length
    : 0;
    ...
    <div className="anim-fade-in" style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <p style={{ fontSize: 14, color: "#64748b", margin: "4px 0 0" }}>
          Real provider routing only. Featherless now leads the heavy intelligence path.
        </p>
      </div>
      ...
      >
          ...
          <div>
            ...
            {!loading && !error && !health?.any_available && (
              <p style={{ fontSize: 12, color: "#b45309", margin: "2px 0 0" }}>
                Configure FEATHERLESS_API_KEY, GROQ_API_KEY or HF_API_KEY in backend/.env
              </p>
            )}
          </div>
        </div>
        {!loading && health && (
          <div style={{ display: "flex", gap: 6 }}>
            {[health.featherless, health.groq, health.huggingface, health.ollama].map((ok, i) => (
