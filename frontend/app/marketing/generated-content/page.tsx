...
export default function MarketingGeneratedContentPage() {
  ...
  const handleDownload = (campaign: MarketingCampaign) => {
    const body = formatCampaignBody(campaign);
    const blob = new Blob([body || JSON.stringify(campaign.content, null, 2)], { type: "text/plain;charset=utf-8" });
              ...
              >
                ...
                <div style={{ borderRadius: 20, background: "#fff", border: "1px solid #e2e8f0", padding: 18, display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <div style={{ whiteSpace: "pre-wrap", color: "#334155", fontSize: 14, lineHeight: 1.8 }}>
                    {body || truncateClean(JSON.stringify(campaign.content, null, 2), 1200)}
                  </div>
                </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: "auto" }}>
                  ...
                  <Link href={`/marketing?tab=campaigns&campaign_id=${campaign.id}&business_id=${businessId}`} style={linkActionButton("#f8fafc", "#0f172a")}>
                    <Sparkles size={14} />
                    Edit
                  </Link>
                  <Link href={`/marketing?tab=campaigns&campaign_id=${campaign.id}&action=publish&business_id=${businessId}`} style={linkActionButton("#ede9fe", "#6d28d9")}>
                    <Share2 size={14} />
                    Post
                  </Link>
