...
import { Activity, Zap, Cloud, Cpu, AlertTriangle, CheckCircle, XCircle, RefreshCw, TrendingUp, Clock, BarChart3, Shield, Radio, GitBranch, RotateCcw } from "lucide-react";
...
const ACTIVE_PROVIDERS = new Set(["groq", "huggingface", "ollama"]);
const isActiveProvider = (provider: string) => ACTIVE_PROVIDERS.has(provider);
const PROVIDER_ICONS: Record<string,any> = { groq: Zap, huggingface: Cloud, ollama: Cpu };
const PROVIDER_COLORS: Record<string,string> = { groq: "#6366f1", huggingface: "#f59e0b", ollama: "#10b981" };
...
export default function OpsPage() {
  ...
  const providers = data ? Object.values(data.providers).filter((provider) => isActiveProvider(provider.name)) : [];
  const sys = data?.system;
  const failures = (data?.recent_failures || []).filter((failure) => isActiveProvider(failure.provider));
  const activeLatencyProviders = latencyData
    ? Object.entries(latencyData.providers).filter(([name]) => isActiveProvider(name))
    : [];
  const activeFallbackEvents = fallbackData
    ? fallbackData.events.filter((event) => isActiveProvider(event.from_provider) && isActiveProvider(event.to_provider))
    : [];
  const activeRoutingProviders = routingData
    ? routingData.providers.filter((provider) => isActiveProvider(provider.provider))
    : [];
      ...
      {latencyData && (
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e2e8f0", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {activeLatencyProviders.map(([name, lp]) => {
        ...
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e2e8f0", overflow: "hidden" }}>
          ...
          {activeFallbackEvents.length === 0 ? (
            ...
            <div style={{ maxHeight: 300, overflowY: "auto" }}>
              {activeFallbackEvents.slice(0, 20).map((ev, i) => (
      ...
      {routingData && (
        <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #e2e8f0", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {activeRoutingProviders.map((rp, i) => (
